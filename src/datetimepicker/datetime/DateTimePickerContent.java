...
public class DateTimePickerContent extends VBox {
    ...
    DateTimePickerContent(final DateTimePicker dateTimePicker) {
    ...
    void updateGrid() {
        ...
        int nCols = daysPerWeek + (dateTimePicker.isShowWeekNumbers() ? 1 : 0);
    ...
    void updateDayCells() {
        ...
        for (int i = 0; i < 6 * daysPerWeek; i++) {
            ...
            try {
                ...
                dayCellDates[i] = LocalDateTime.of(date, LocalTime.MIDNIGHT);
                ...
                if (date.equals(dateTimePicker.getValue())) {
    ...
    protected LocalDateTime dayCellDate(DateCell dateCell) {
    ...
    public void goToDayCell(DateCell dateCell, int offset, ChronoUnit unit, boolean focusDayCell) {
        goToDate(dayCellDate(dateCell).toLocalDate().plus(offset, unit), focusDayCell);
    }
    ...
    public void selectDayCell(DateCell dateCell) {
        dateTimePicker.setValue(dayCellDate(dateCell));
        dateTimePicker.hide();
    }
    ...
    void clearFocus() {
        LocalDate focusDate = dateTimePicker.getValue().toLocalDate();
        if (focusDate == null) {
    ...
    private DateCell createDayCell() {
        ...
        if (dateTimePicker.getDayCellFactory() != null) {
            cell = dateTimePicker.getDayCellFactory().call(dateTimePicker);
        }
        if (cell == null) {
            cell = new DateCell();
        }
